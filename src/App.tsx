...
/** Try Simple English Wikipedia first for ELI5. */
...
export default function App() {
  ...
  // When a card opens, fetch the answer + media on demand
  ...
  const makeEli5 = async () => {
    ...
    try {
      const baseTitle = selectedQA.titleGuess || selectedQA.question.replace(/^What is\s+|\?$/gi, "");
      const simple =
        (baseTitle && (await fetchSimpleSummaryBySearch(baseTitle))) ||
        (await fetchSimpleSummaryBySearch(selectedQA.question));
      const simplified = simple ?? localEli5(selectedQA.answer || "");
      setEli5Text(simplified);
      setShowEli5(true);
    } finally {
  ...
  const makeOverviewEli5 = async () => {
    ...
    try {
      const key = overviewTitle || query.trim();
      const simple = await fetchSimpleSummaryBySearch(key);
      const simplified = simple ?? localEli5(overview);
      setOverviewEli5Text(simplified);
  ...
  return (
    <div className="min-h-screen bg-black text-white antialiased selection:bg-white/10">
          ...
          {view === "results" && (
            <form onSubmit={onSubmit} className="w-full max-w-md" aria-label="Search form">
              <div className="relative flex items-center gap-2 rounded-xl border border-white/15 bg-white/[0.05] p-2">
                <svg aria-hidden className="ml-2 h-5 w-5 text-white/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                />
                <button type="submit" disabled={isLoading} className="rounded-lg bg-white text-black px-3 py-2 text-xs font-medium hover:bg-white/90 disabled:opacity-60 disabled:cursor-not-allowed">
            ...
            <form onSubmit={onSubmit} className="group relative" aria-label="Search form">
              ...
              <div className="relative flex items-center gap-2 rounded-2xl border border-white/15 bg-white/[0.03] p-2 backdrop-blur-sm shadow-[0_0_0_1px_rgba(255,255,255,0.06)] hover:border-white/25 focus-within:border-white/30">
                ...
                <input
                  ...
                  onChange={(e) => setQuery(e.target.value)}
                />
                <kbd className="mr-2 hidden rounded-lg border border-white/15 bg-white/5 px-2 py-1 text-[10px] text-white/60 sm:inline-block">/</kbd>
                <button type="submit" disabled={isLoading} className="rounded-xl bg-white text-black px-3 py-2 text-sm font-medium hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-white/30 disabled:opacity-60 disabled:cursor-not-allowed">
                  {isLoading ? "Searching…" : "Search"}
                </button>
              </div>
            </form>
        ...
        <main className="mx-auto max-w-6xl px-4 pt-24 pb-20">
          ...
          {notFound && (
            <div className="mb-6 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <p className="mt-1 text-xs text-white/70">We couldn’t find results for “{query}”. Perhaps you meant:</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {suggestions.length > 0 ? suggestions.map((s) => (
                  <button key={s} onClick={() => { setQuery(s); fetchOverviewAndQuestions(s); }} className="rounded-full border border-white/15 bg-white/[0.06] px-3 py-1.5 text-xs hover:bg-white/[0.12]">
                    {s}
                  </button>
          ...
          <section className="relative rounded-2xl border border-white/10 bg-white/[0.03] p-5 min-h-[25svh]" style={{ minHeight: "25vh" }}>
            {isLoading ? (
      ...
      {/* Modal */}
      {selectedQA && (
        <Modal onClose={() => setSelectedQA(null)} title={selectedQA.question}>
          {/* Body */}
          {isModalLoading ? (
            <div className="animate-pulse space-y-2">
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Tab") {
        ...
        else if (isShift && document.activeElement === first) { e.preventDefault(); last.focus(); }
      }
    };
